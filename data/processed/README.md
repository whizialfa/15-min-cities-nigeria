...
`settlements.csv` is named GRID3 (and OSM village/hamlet) points clipped to each `{slug}_study_boundary.gpkg`. Coordinates only. Not drawn on the print plates. Per-city files are `{slug}_settlements.csv`. The live map search index is `web/data/settlements_search.json`.
