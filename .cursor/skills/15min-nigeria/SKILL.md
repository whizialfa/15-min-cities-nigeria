...
This is **not** a Nature Cities clone. GRID3 health/education on metro LGAs, dual access **n = 5**, OSM walk at 5 km/h. Quote F15 from `city_metrics.csv` walk rows (`network=walk`, `n_required=5`). Plate cuts (Lagos seven LGAs, Abuja seven wards) live in `plate_metrics.csv` and are never the metro headline. Never quote Euclidean F15 as a result. Never drop no-POI hexes silently.
...
- Boundary: metro LGAs in `cities.py` (Ibadan is the **core 5** LGAs, not 11). GHS Urban Centre is a missing sensitivity, not a clone of Bruno.
...
- Walk headline: OSM **walk** graph at 5 km/h (not Euclidean). Euclidean stays as `PT_eucl`. Euclidean overstates F15 everywhere, and worst where the street network is most broken: Lagos 95.5% → 85.4%, Ibadan 92.8% → 82.1%, Kano 82.2% → 67.2%, Abuja 40.6% → 16.5%, Port Harcourt 68.5% → 30.7%. Never quote `F15_eucl` as a result.
...
**Abuja is the sole exception and the only city where N\* exceeds the existing stock.** It needs 284 against 238 held, at 13.1 per 100k — nearly three times any other city — because population is spread over 1,476 km². Read that number next to the completeness mask: 50.9% of Abuja's hexes are off the mapped walk network, so part of the gap is unmapped street, not absent clinic.

N\* per 100k is an inverse density reading: Kano 2.1 (compact) through to Abuja 13.1 (sprawled). It assumes a facility can be sited at any hex centroid on the walk graph, so it ignores land, cost and catchment capacity — it is a floor on siting efficiency, not a build plan.
...
**At Bruno's n = 20, Abuja scores 0.0% and Port Harcourt 1.1%.** That is a statement about the parameter, not the city: n = 20 presumes dense substitutable amenities and does not transfer. This is the empirical case for the n = 5 headline — make the argument with this table, not by assertion. Do not present n = 20 as the result.

**5 km/h is an optimistic walk.** Dropping to 3.5 km/h costs Lagos 17 points, Abuja 12, Port Harcourt 21, Kano 24 (67.2% → 43.0%). Given heat, footpath quality and flooding, quote the speed with every F15 and treat 3.5–4.5 km/h as the honest lower band.
...
What the grids do support is **underserved headcounts**, which is the more useful framing anyway. Roughly **1.2 million under-5s across the five cities live beyond a 15-minute walk of a clinic** (Kano 426k, Abuja 277k, Lagos 233k, Port Harcourt 228k, Ibadan 31k).
...
**Street network** — distance from hex centroid to the nearest walk-graph node (`SNAP_WARN_M = 250`). Ibadan is the cleanest mapped city (median 47 m, p90 121 m, 0.7% of hexes off-network). Lagos looks bad by hex share (24.8%) but those hexes hold 2.8% of people — lagoon and empty land, not a mapping gap. **Abuja is the one real warning: median snap 263 m, p90 16.7 km, 50.9% of hexes off-network holding 7.0% of the population.** Abuja's F15 of 16.5% is therefore part genuine sprawl and part unmapped street network, and must never be quoted bare.

**POI inventory** — OSM under-records schools everywhere: ratios against GRID3 over the same bbox run 0.066 (Ibadan), 0.079 (Lagos), 0.140 (Abuja), 0.173 (Port Harcourt), 0.193 (Kano). OSM is under a fifth of GRID3 in every city, which confirms the existing policy — GRID3 primary, OSM last-resort overlay — rather than showing a north–south bias. If anything the ratio runs the other way, because GRID3 itself records far more (largely private) schools in the south.
...
- **The completeness mask is texture, never tint — and not on the QGIS plates.** If a separate matplotlib mask figure is needed, hatch the dissolved off-network region (1 m buffer before union) rather than veiling the ramp. Quote `completeness.csv` with every F15; Abuja is the warning (50.9% of hexes off-network).
- The matplotlib colourbar label sits **above** the bar; its ticks used to collide with the vintage line.

Mask coverage as shipped: Ibadan 0.7% of hexes, Port Harcourt 13.3%, Kano 18.6%, Lagos 24.8% (Lekki and the lagoon fringe — 2.8% of people), Abuja 50.9%.

Plates do **not** draw metro LGAs. Wards stay as outlines: GRID3 operational/vaccination wards clipped to the metro LGAs (`{slug}_wards.gpkg` via `proximity.wards`). **Print labels are OSM named places** (`{slug}_places.gpkg` via `proximity.places`): `place=suburb|quarter|neighbourhood` plus named residential — Apo, Lokogoma, Ikoyi, Sangotedo. Helvetica 14 pt, `PreventOverlap`. Pinned names win PAL priority. **Abuja plates draw only seven wards** — Gwarinpa, Wuse, Nyanya, Karu, City Centre, Garki, Kabusa — as the black outline and the hex clip. Not AMAC, not Gui/Orozo/Gwagwa/Jiwa. **Lagos plates draw seven LGAs** — Eti-Osa, Lagos Island, Apapa, Lagos Mainland, Surulere, Mushin, Shomolu — not the 16-LGA metro. Stroke 0.22 mm, `90,84,78,165`. Rivers wards with no settlement name keep `PH 6` in the data.
...
Abuja is the one city where the metro LGA is mostly not city: AMAC is 1,476 km² at 1,470 people/km², against 6,946 (Port Harcourt) to 10,092 (Kano). Half its hexes hold 0.1% of its people. This does **not** bias `F15`, which is population-weighted — F15 over populated hexes only is identical to 0.165 — but it does dominate the hex choropleth, so the Abuja map reads as mostly empty. Treat that as cartography to fix, not a metric to correct.
